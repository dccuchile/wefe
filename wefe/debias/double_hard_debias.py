...
class DoubleHardDebias(BaseDebias):
    ...
    def bias_by_projection(self, 
        ...
        for word in model.vocab:
            ...
            similarities[word] = self.similarity(embedding,word1) - self.similarity(embedding,word2)
        return similarities
    ...
    def principal_components(self, model:WordEmbeddingModel,incremental_pca:bool) -> np.ndarray:
        if incremental_pca:
            pca = IncrementalPCA() 
        else: 
    ...
    def transform(
        ...
        female, male = self.get_target_words(model,ignore,n_words,bias_representation)
