...
class WEAT(BaseMetric):
    ...
    def _calc_p_value(self, target_embeddings: List[EmbeddingDict],
                      attribute_embeddings: List[EmbeddingDict], original_score: float,
                      iterations: int, method: Literal['approximate', 'exact'],
                      test_type: Literal['left-sided', 'right-sided',
                                         'two-sided'], verbose: bool):
        ...
        if method != 'approximate':
            raise Exception(f'p value method should be "exact", "approximate"'
                            f', got {method}.')
        ...
        if test_type == 'left-sided':
            ...
            raise Exception(f'p value test type should be "left-sided", "right-sided" '
                            f'or "two-sided", got {test_type}')

        if not isinstance(iterations, (int, float)):
            raise TypeError(f'p value iterations should be int instance, '
                            f'got {iterations}.')
        ...
        while len(permutations_seen) < runs:
            ...
            permutation = tuple(
                np.random.choice(pool_target_words,
            ...
            if permutation not in permutations_seen:
                ...
                curr_permutation_weat = self._calc_weat(X_i, Y_i, attribute_0,
                                                        attribute_1)
        ...
        p_value = (count_pass_function + 1) / (runs + 1
    ...
    def run_query(self,
                  ...
                  p_value_test_type: Literal['left-sided', 'right-sided',
