...
def test_ran_debias_class(model, capsys):
    ...
    target = (
        weat_wordset["pleasant_5"]
        + weat_wordset["unpleasant_5"]
        + weat_wordset["career"]
        + weat_wordset["family"]
    )
    gender_debiased_w2v = ran.transform(
        model, target=target, ignore=gender_specific, copy=True
    )
    assert model.name == "word2vec"
    assert gender_debiased_w2v.name == "word2vec_gender_debiased"
    ...
    targets = weat_wordset["male_names"] + weat_wordset["female_names"]
    attributes = weat_wordset["pleasant_5"] + weat_wordset["unpleasant_5"]
    gender_debiased_w2v = ran.fit(model, definitional_pairs).transform(
        model,
        target=attributes,
        ignore=gender_specific + targets + attributes,
        copy=True,
    )
    ...
    gender_debiased_w2v = ran.fit(model, definitional_pairs).transform(
        model, target=attributes, ignore=gender_specific, copy=True
    )
    ...
    assert "Identifying the bias subspace" in out
    assert (
        f"Executing Repulsion attraction Neutralization Debias on {model.name}" in out
    )
    assert "Copy argument is True. Transform will attempt to create a copy" in out
    ...
    gender_debiased_w2v = ran.transform(
        model, target=attributes, ignore=gender_specific, copy=False
    )
    assert model == gender_debiased_w2v
    assert model.wv == gender_debiased_w2v.wv
    assert model.name == gender_debiased_w2v.name
